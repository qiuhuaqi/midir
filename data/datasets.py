...
class _BaseDataset(Dataset):
    ...
    def __init__(self, data_dir_path):
        ...
        assert os.path.exists(
            data_dir_path
        ), f"Data dir does not exist: {data_dir_path}"
...
class BrainMRInterSubj3D(_BaseDataset):
    def __init__(
        self, data_dir_path, crop_size, evaluate=False, modality="t1t1", atlas_path=None
    ):
    ...
    def _set_path(self, index):
        ...
        if self.evaluate:
            ...
            self.data_path_dict[
                "target_original"
            ] = f"{self.src_subj_path}/T1_brain.nii.gz"
            self.img_keys.append("target_original")
            # segmentation
            self.data_path_dict[
                "target_seg"
            ] = f"{self.tar_subj_path}/T1_brain_MALPEM_tissues.nii.gz"
            self.data_path_dict[
                "source_seg"
            ] = f"{self.src_subj_path}/T1_brain_MALPEM_tissues.nii.gz"
...
class CardiacMR2D(_BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        data_dict = _magic_slicer(
            data_dict, slice_range=self.slice_range, slicing=self.slicing
        )
        data_dict = _crop_and_pad(data_dict, self.crop_size)
        data_dict = _normalise_intensity(data_dict, self.img_keys)
        return _to_tensor(data_dict)
