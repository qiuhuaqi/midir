...
class BrainInterSubject(_BaseDataset):
    def __init__(self, data_dir, run, dim, crop_size, atlas_path=None, slice_range=(70, 90)):
        super(BrainInterSubject, self).__init__(data_dir, run, dim, slice_range=slice_range)
    ...
    def _set_path(self, index):
        ...
        if self.atlas_path is not None:
            ...
            tar_subj_path = f"{self.data_dir}/{tar_subj_id}"
        else:
            tar_subj_path = self.atlas_path

        src_subj_id = random.choice(self.subject_list)
        src_subj_path = f"{self.data_dir}/{src_subj_id}"

        data_path_dict = dict()
        data_path_dict["target"] = f"{tar_subj_path}/T1_brain.nii.gz"
        data_path_dict["source"] = f"{src_subj_path}/T2_brain.nii.gz"
        # data_path_dict["roi_mask"] = f"{tar_subj_path}/T1_brain_mask.nii.gz"

        if self.run == 'eval':
            data_path_dict["target_seg"] = f"{tar_subj_path}/T1_brain_MALPEM_tissues.nii.gz"
            data_path_dict["source_seg"] = f"{src_subj_path}/T1_brain_MALPEM_tissues.nii.gz"
    ...
    def __getitem__(self, index):
        ...
        data_dict = self._normalise_intensity(data_dict, ['target', 'source'])
